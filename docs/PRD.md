...
Stage 1 supplied the Worker/D1, hashed development sessions, owner-isolated versioned CRUD, and pure once/interval scheduling core. Stage 2A encrypted transport is implemented and locally verified with 117 tests and a built-Worker encrypted HTTP round-trip. See `VERIFICATION.md`. No remote CI success, Android build, complete E2EE application, or deployment is claimed.

Flutter is not installed and the Android application is absent. No cloud deployment resources or production identity have been provisioned. Stage 2A adds encrypted envelope transport and a memory-only WebCrypto reference, not a complete E2EE app. Other full-product capabilities remain future requirements.
...
Android ONLY, finger handwriting WITHOUT a stylus, and E2EE REQUIRED are confirmed. Flutter is the proposed client technology. iOS, APNs, macOS workflows, stylus support, pressure features, and palm rejection are out of scope, not pending. The real Android test model and minimum OS remain to be selected. A future browser clipper remains in scope but does not imply a desktop or complete browser editor.
...
**Do not store important data: `generateNoteKey()` is nonexportable and memory-only. Process exit can make content unrecoverable.** No secure vault/persistence, wrapping, recovery, device pairing, key authentication, or encrypted sharing is implemented. A session key does not decrypt data; OAuth account recovery is not data recovery. Ciphertext storage alone does not deliver complete E2EE.
...
The proposed next key-management gates are a random vault root, wrapped per-document keys, Android Keystore device wrapping, a user-held offline recovery code, and trusted-device transfer, with no server master key. These are not delivered. Approve recovery UX and test recovery before real data use. Losing all trusted devices and recovery material makes content unrecoverable; OAuth account recovery cannot fix that.
...
- Android test model and minimum OS; later browser-clipper targets and Android distribution method. Android-only and finger-only ink are settled.
